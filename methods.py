...
def get_misspelled(dictionary, words):
    ...
    for word in words: #getting all words wrong or not
        word = rem_punct(word)
        #filtering mispelled words:-
        if is_misspelled(dictionary, word) and \
                word not in misspelled_words:
...
def rem_punct(word):
    ...
    """
    if word[0] in "&-":  # may return an empty word
        word = word[1:]
...
def is_misspelled(dictionary, word):
    ...
    if len(word) == 0 or word.isdigit(): #Handling the empty words from rem_punct and handling numbers / word.isdigit()built in method return true if the word is a number
        return False
    else:
...
def get_suggestions(dictionary, misspelled_words, n=3):
    ...
    for word in misspelled_words:
        ...
        temp = []
        s = SequenceMatcher()
        s.set_seq2(word)  # set the first value

        for item in dictionary:
            s.set_seq1(item)  # set the second value

            if s.real_quick_ratio() >= 0.65 and \
                    s.quick_ratio() >= 0.65 and \
                    s.ratio() >= 0.65:  # comparing the first value with the second one
...
def selection_sort(alist, n2):  # n2 is the number of suggestions

    n = len(alist)

    if n2 > n: #if the number of the required suggestions is bigger than the available suggestions.
        n2 = n
